...
public class AllJoynService extends Service implements Observer {
	...
	public void onCreate() {
		...
		CharSequence title = "Music Player";
		CharSequence message = "Player hosting service";
	...
	public synchronized void update(Observable o, Object arg) {
		...
		if (qualifier.equals(MusicPlayerApplication.USE_JOIN_CHANNEL_EVENT)) {
			Message message = mHandler.obtainMessage(HANDLE_USE_JOIN_CHANNEL_EVENT);
			mHandler.sendMessage(message);
		}

		if (qualifier.equals(MusicPlayerApplication.USE_LEAVE_CHANNEL_EVENT)) {
			Message message = mHandler.obtainMessage(HANDLE_USE_LEAVE_CHANNEL_EVENT);
			mHandler.sendMessage(message);
		}

		if (qualifier.equals(MusicPlayerApplication.HOST_INIT_CHANNEL_EVENT)) {
			Message message = mHandler.obtainMessage(HANDLE_HOST_INIT_CHANNEL_EVENT);
			mHandler.sendMessage(message);
		}

		if (qualifier.equals(MusicPlayerApplication.HOST_START_CHANNEL_EVENT)) {
			Message message = mHandler.obtainMessage(HANDLE_HOST_START_CHANNEL_EVENT);
			mHandler.sendMessage(message);
		}

		if (qualifier.equals(MusicPlayerApplication.HOST_STOP_CHANNEL_EVENT)) {
			Message message = mHandler.obtainMessage(HANDLE_HOST_STOP_CHANNEL_EVENT);
			mHandler.sendMessage(message);
		}

		if (qualifier.equals(MusicPlayerApplication.OUTBOUND_CHANGED_EVENT)) {
	 ...
	 * this obejct.
	 */
	private BusAttachment mBus = new BusAttachment(MusicPlayerApplication.PACKAGE_NAME, BusAttachment.RemoteMessage.Receive);
	...
	private void doConnect() {
		...
		if (Status.OK != status) {
			mChatApplication.alljoynError(MusicPlayerApplication.Module.HOST, "Unable to register the chat bus object: (" + status + ")");
			return;
		}

		status = mBus.connect();
		if (status != Status.OK) {
			mChatApplication.alljoynError(MusicPlayerApplication.Module.GENERAL, "Unable to connect to the bus: (" + status + ")");
			return;
		}

		status = mBus.registerSignalHandlers(this);
		if (status != Status.OK) {
			mChatApplication.alljoynError(MusicPlayerApplication.Module.GENERAL, "Unable to register signal handlers: (" + status + ")");
			return;
		}
	...
	private void doStartDiscovery() {
		...
		if (status == Status.OK) {
			...
			mChatApplication.alljoynError(MusicPlayerApplication.Module.USE, "Unable to start finding advertised names: (" + status + ")");
			return;
		}
	}
	...
	private void doRequestName() {
		...
		if (status == Status.OK) {
			...
			mChatApplication.alljoynError(MusicPlayerApplication.Module.USE, "Unable to acquire well-known name: (" + status + ")");
		}
	}
	...
	private void doBindSession() {
		...
		Status status = mBus.bindSessionPort(contactPort, sessionOpts, new SessionPortListener() {
			/**
			 * This method is called when a client tries to join the session we
			 * have bound. It asks us if we want to accept the client into our
			 * session.
			 * 
			 * In the class documentation for the SessionPortListener note that
			 * it is a requirement for this method to be multithread safe. Since
			 * we never access any shared state, this requirement is met.
			 */
		...
		if (status == Status.OK) {
			...
			mChatApplication.alljoynError(MusicPlayerApplication.Module.HOST, "Unable to bind session contact port: (" + status + ")");
			return;
		}
	}
	...
	private void doAdvertise() {
		...
		if (status == Status.OK) {
			...
			mChatApplication.alljoynError(MusicPlayerApplication.Module.HOST, "Unable to advertise well-known name: (" + status + ")");
			return;
		}
	}
	...
	private void doCancelAdvertise() {
		...
		if (status != Status.OK) {
			mChatApplication.alljoynError(MusicPlayerApplication.Module.HOST, "Unable to cancel advertisement of well-known name: (" + status + ")");
			return;
		}
	...
	private void doJoinSession() {
		...
		Status status = mBus.joinSession(wellKnownName, contactPort, sessionId, sessionOpts, new SessionListener() {
			 ...
			 * requirement for this method to be multithread safe. This is
			 * accomplished by the use of a monitor on the ChatApplication as
			 * exemplified by the synchronized attribute of the
			 * removeFoundChannel method there.
			 */
			public void sessionLost(int sessionId, int reason) {
				Log.i(TAG, "BusListener.sessionLost(sessionId=" + sessionId + ",reason=" + reason + ")");
				mChatApplication.alljoynError(MusicPlayerApplication.Module.USE, "The chat session has been lost");
		...
		if (status == Status.OK) {
			...
			mChatApplication.alljoynError(MusicPlayerApplication.Module.USE, "Unable to join chat session: (" + status + ")");
			return;
		}
	...
	private void doSendMessages() {
		...
		while ((message = mChatApplication.getOutboundItem()) != null) {
			...
			try {
				...
				mChatApplication.alljoynError(MusicPlayerApplication.Module.USE, "Bus exception while sending message: (" + ex + ")");
			}
		}
	}
